...
def next_invoice_number(invoice_number: str) -> str:
    ...
    # print(new_invoice_number)   # TODO Delete after testing
...
def record_invoice(invoice_file: TextIO,
    ...
    """
    # last_line_ptr = 0
    invoice_file.seek(last_line_ptr, SEEK_SET)    # The seeking to the start of the file each time loop runs, then seek
                                                   # to that position the next time we call function.
    last_row = ''
    for line in invoice_file:
        print('*', end='')  # TODO Delete after testing
        last_row = line
        # print(last_row)     # TODO Delete after testing
    if last_row:
        ...
        # if file is empty, we'll start numbering from 1
        year = get_year()
        new_invoice_number = f'{year}-{1:04d}'

    last_line_ptr = invoice_file.tell()   # we stored the position of the file pointer before adding a  new line
...
with open(data_file, 'r+') as invoices:
    last_line = record_invoice(invoices, 'ACME Roadrunner', 18.40)  # we pass the file pointer position
    last_line = record_invoice(invoices, 'Squirrel Storage', 320.55, last_line)
     #  record_invoice will use that position, to seek directly to the last line in the file.
